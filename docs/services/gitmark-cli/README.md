...
deletes and rebuilds the tables unconditionally.
...
to cwd. See `repo_root()`. So `gitmark` can be run from anywhere inside
...
`.venv`, `venv`, `vendor`, and `.gitmark` itself (`EXCLUDE_DIRS`).
On top of that, `.gitignore`d paths are excluded — a hand-written subset parser
(`parse_gitignore`: folder names with a trailing `/`, exact names and
...
`.gitmark/index.db` under the repo root (`DB_REL`). It is a derived cache and
...
2. For each file: derive a `title` (first `# H1`, else filename — `title_of`) and an `area`
   (`area_of` — `docs/services/<svc>` gets its own group, otherwise `docs/<sub>`,
   `services/<svc>`, etc.).
3. **Chunking** — `chunk_md()` splits each file at markdown headings
...
5. **Links** — `LINK_RE` finds markdown links `[text](href)` (ignoring image links), and
   `resolve_link()` resolves each `href` to a known `.md` file. It
...
`tokenize='trigram'` (`_has_trigram`). If the SQLite build lacks the
...
`cmd_search()` merges up to three passes into one result set keyed by
...
1. **bm25 (exact terms)** — query words ≥2 chars become a prefix `OR` match
   (`"term"* OR …`, `_fts_match_query`) against `fts`. Score is
...
3. **fuzzy (n-gram)** — handles typos, morphology, and Cyrillic. `_fuzzy_phrases()` breaks
   query words (≥4 chars) into overlapping 4-char windows; these are OR-matched against
   `tri`. A chunk is accepted only if its body contains at least `need` distinct windows
   (≈ceil(20% of windows), min 1), which filters chunks that merely share one common gram.
   Score weighted ×0.3. `via: "fuzzy"`.
...
`cmd_inventory()` regenerates the two summary tables in `docs/reference/commands.md`
between the `<!-- BEGIN/END inventory:commands -->` and `<!-- BEGIN/END inventory:skills -->`
markers — the commands table from the frontmatter (`description`/`args`/`drives`) of the
commands of **both** the project (`<проект>/.omp/commands/*.md`, `_scan_commands`) and the
package that owns the engine (`<пакет>/commands/*.md`), the skills table from
`name`/`description` of the matching `skills/*/SKILL.md` (`_scan_skills`). The two sets are
merged, the project wins on a name collision. Writing happens **only**
between the markers; the hand-written `## /cmd` sections are never touched — and they are
required only for the project's own commands, since the package documents its own. Idempotent:
a second run changes nothing. `inventory_issues()` is the single
...
`cmd_map()` produces one standalone HTML file (`_MAP_HTML` template) with the data inlined as JSON: a collapsible per-area file tree, the
