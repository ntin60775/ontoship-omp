...
description: A relentless interview to sharpen a plan or design, which also builds the project's domain model (CONTEXT.md glossary + decision docs) as it goes, ending in a parent ship contract. Use when the user wants to stress-test a plan or idea before implementing.
...
2. Create/update the plan folder `docs/plans/<slug>/` — the parent contract is
   `docs/plans/<slug>/README.md`: `node_type: plan`, `status: draft`, body fields
   `Goal`, `Done`, `Scope`, `Constraints`, `Context` (see `kb-curate`). No `Tasks`
   section — the decomposition is the tickets, added later by `mp-to-tickets`.
